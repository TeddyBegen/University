...
def update_sat(x1,x2,v1,v2,F,dt):
    ...
    which is modeled as a totally inelastic collision (fullstandigt inelastisk 
stot).
    ...
    if abs(v2-v1)<2 and abs(x2-x1)<5:
        docked = 1
        vnew2=(m1/(m1+m2))*vnew1
        vnew1=vnew2

    elif abs(v2-v1)>2 and abs(x2-x1)<5:
...
while telapsed <= t_lim:
    ...
    if docked == 1:
        ax.text(0.5, 0.2, textstring, transform=ax.transAxes, color="white", 
fontsize=12,
        verticalalignment='top')
